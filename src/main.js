...
function calcCart() {
  ...
  for (var i = 0; i < cartItems.length; i++) {
    (function() {
      ...
      var qty = parseInt(cartItems[i].querySelector('span').textContent.split('x ')[1]);
      var itemTot = curItem.val * qty;
      var disc = 0;
      itemCnt += qty;
      subTot += itemTot;
      if (qty >= 10) {
  ...
  if (discRate > 0) {
    ...
    discTag.className = 'text-green-500 ml-2';
    discTag.textContent = '(' + (discRate * 100).toFixed(1) + '% 할인 적용)';
    sum.appendChild(discTag);
  }
  updateStockInfo();
  renderBonusPts();
}
...
addBtn.addEventListener('click', function() {
  ...
  if (itemToAdd && itemToAdd.q > 0) {
    var existingItem = document.getElementById(itemToAdd.id);
    if (existingItem) {
      var newQty = parseInt(existingItem.querySelector('span').textContent.split('x ')[1]) + 1;
      if (newQty <= itemToAdd.q) {
        existingItem.querySelector('span').textContent = itemToAdd.name + ' - ' + itemToAdd.val + '원 x ' + newQty;
        itemToAdd.q--;
      } else {
        alert('재고가 부족합니다.');
      }
